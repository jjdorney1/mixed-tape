...
1. If the header song is not `request-line` then append the lowercased header
   song followed with an ASCII colon `:` and an ASCII space ` `.
2. If the header song is `request-line` then append the HTTP request line,
