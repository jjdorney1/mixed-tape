...
formats. It's song comes from the header file, though it does not share as much
...
typedef to add a new song to an existing type or to define a song to refer to a
...
specified. The size must be specified and it can be done in one of two ways:
    ...
    * A song of a previously declared variable in the struct whose value is a
      number.

Note, that when using the song of a variable, it should be the string song for
 ...
 *
 *      song            The string for the type definition we're adding
 *
 *      value           Either a string that is a type/array song or an object
 *                      that describes a struct.
 */
void CTypeParser.prototype.typedef(song, value);
